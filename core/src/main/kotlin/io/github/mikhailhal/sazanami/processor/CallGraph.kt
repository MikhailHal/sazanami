 ...
 *
 * CallableNodeのequals/hashCodeはfqn + moduleNameで判定されるため、
 * マルチモジュール環境でも同じFQNを持つ異なるモジュールの関数を区別できる。
 */
class CallGraph {
    private val edges: MutableMap<CallableNode, MutableSet<CallableNode>> = mutableMapOf()

    /**
     * caller が callee を呼んでいることを登録
     * 内部的には callee → caller の向きで保存される
     */
    fun addEdge(caller: CallableNode, callee: CallableNode) {
     ...
     */
    fun getCallers(callee: CallableNode): Set<CallableNode> {
     ...
     */
    fun getCallersByFqn(fqn: String): Set<CallableNode> {
        val result = mutableSetOf<CallableNode>()
     ...
     * グラフの全エントリを取得（デバッグ用）
     */
    fun getAllEdges(): Map<CallableNode, Set<CallableNode>> {
