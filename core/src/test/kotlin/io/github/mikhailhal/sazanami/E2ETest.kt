...
class E2ETest {
    ...
    fun `changing Calculator_add detects testAdd and testHelper`() {
        withSandboxFiles { ktFiles ->
            ...
            assertEquals(
                """
                io.github.mikhailhal.sazanami.CalculatorTest.testAdd
                io.github.mikhailhal.sazanami.CalculatorTest.testHelper
                """.trimIndent(),
                output
            )
        }
    ...
    fun `changing helperB detects only testHelper`() {
        withSandboxFiles { ktFiles ->
            ...
            assertEquals("io.github.mikhailhal.sazanami.CalculatorTest.testHelper", output)
        }
    }
    ...
    fun `changing function in moduleA detects test in moduleB that calls it`() {
        withCrossModuleFiles { moduleFiles, pathMapping ->
            ...
            assertEquals("io.github.mikhailhal.sazanami.moduleb.AppServiceTest.testExecute", output)
        }
    }
    ...
    fun `changing implementation method detects tests calling interface method`() {
        withSandboxFiles { ktFiles ->
            ...
            assertEquals("io.github.mikhailhal.sazanami.InterfaceTest.testCompute", output)
        }
    }
