...
class ChangedFunctionCollectorTest {
    ...
    fun `collects changed function when diff overlaps function range`() {
        ...
        try {
            ...
            assertEquals(
                setOf(ChangedFunction("io.github.mikhailhal.sazanami.Calculator.add", testModule)),
                changed
            )
        } finally {
    ...
    fun `collects multiple changed functions`() {
        ...
        try {
            ...
            assertEquals(
                setOf(
                    ChangedFunction("io.github.mikhailhal.sazanami.Calculator.add", testModule),
                    ChangedFunction("io.github.mikhailhal.sazanami.Calculator.multiply", testModule)
                ),
                changed
            )
        } finally {
    ...
    fun `collects top-level function`() {
        ...
        try {
            ...
            assertEquals(setOf(ChangedFunction("io.github.mikhailhal.sazanami.helperB", testModule)), changed)
        } finally {
            Disposer.dispose(projectDisposable)
        }
    }
